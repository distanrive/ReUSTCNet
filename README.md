...
从 [Releases](https://github.com/yourname/ReUSTCNet/releases) 页面下载最新的 `reustcnet.exe`，双击运行。  
...
```bash
# 克隆仓库
git clone https://github.com/yourname/ReUSTCNet.git
cd ReUSTCNet
...
1. **复制配置文件模板**  
   将 `build_config.bat.example` 复制为 `build_config.bat`。

2. **编辑 `build_config.bat`**，按实际环境修改以下路径：
...
3. **将你的 `icon.ico` 放入项目根目录**（如果使用自定义图标）。

4. **双击 `build.bat`**，等待片刻。生成的 `dist\reustcnet.exe` 即为单文件程序。
...
| `export_type`           | 出口编号：0-1教育网出口, 1-2电信网出口, 2-3联通网出口, 3-4电信网出口2, 4-5联通网出口2, 5-6电信网出口3, 6-7联通网出口3, 7-8教育网出口2, 8-9移动网出口 |
